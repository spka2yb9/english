# -*- coding: utf-8 -*-
"""第146回の描き直し。momentous が「書類にサイン」にしか見えなかった。"""
import sys, os, math
...
add('momentous', '暦の一日だけが赤く大きく囲まれ、その下で人々が手を上げて喜ぶ', f'''
<g transform="translate(300 170)">
  <rect x="-190" y="-140" width="380" height="280" rx="14" class="paper"/>
  <path d="M-190-140h380v50h-380z" class="teal o"/>
  <path d="M-120-140v-24M120-140v-24" fill="none" stroke="{INK}" stroke-width="8" stroke-linecap="round"/>
  {''.join(f'<rect x="{-160+c*62}" y="{-64+r*58}" width="44" height="40" rx="6" fill="#eef2f4"/>' for r in range(3) for c in range(6))}
  <rect x="-36" y="-6" width="44" height="40" rx="6" class="goldp o"/>
</g>
<circle cx="286" cy="184" r="52" fill="none" stroke="{CRL}" stroke-width="8"/>
{''.join(spark(x, y, 1.1) for x, y in [(80,70),(524,60),(540,200)])}
{''.join(f'<g transform="translate({120+i*90} 400)">{person(0, 0, 0.62, 1, c, "blue", "up", h, "smile")}</g>'
         for i, (c, h) in enumerate([('coral','short'),('violet','bob'),('green','short'),('gold','bun'),('blue','cap')]))}''')
