...
export function Illustration({ sceneId }: {
  ...
  return <AnimationPlayer key={sceneId} sceneId={sceneId} animation={animation} />
}

function AnimationPlayer({ sceneId, animation }: {
  sceneId: string
  animation: (typeof LESSON_ANIMATIONS)[string]
}) {
  const titleId = useId()
  const stageId = useId()
