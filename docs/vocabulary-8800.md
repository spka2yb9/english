...
- 出題は `src/services/vocabSelection.ts` の `LEVEL_WEIGHTS`（A2 3 : B1 2 : B2 1）でレベルの低い語を優先する。
  復習枠は半分（5語なら3語）までで、残りは必ず新出語。新出語の先頭1語は一番低いレベルから取るので、
  未出題の A2 が残っている限りどのセッションにも A2 が入る（1,000セッションの試行で A2ゼロの回は 0%）。
  参考: 初回は A2 19% → 50%、B1/B2 だけに再学習中の語が200語ある状態では 8% → 28% になった。
